...
function FloatingWhale({ src, config, gyro }: {
  ...
  // We use motion values so drag and swim animation don't fight each other.
  // The whale starts at its config position; swim offsets are applied on top via
  // separate springed motion values.
  ...
  }, [gyro.x, gyro.y]); // re-run when gyro changes
...
function FloatingWhales() {
  ...
  // Try to start gyro (Android works immediately; iOS needs permission tap)
  const startGyro = () => {
  ...
  useEffect(() => {
    // Android / desktop — just attach directly
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, zIndex: 1, overflow: 'hidden', pointerEvents: 'none' }}>
      ...
      {/* iOS gyro permission button — only shows on iPhones before permission */}
